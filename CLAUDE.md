...
**Tenant-wide variant for Trade Cost.** The same 4-stage pipeline runs at scale in `fetchActualExpenseRollupForAllProjects` (`src/lib/dataverse/actualExpenseRollup.ts`) — one IN-chunked sweep across every active projectNo instead of one project at a time. Output is a flat `ActualExpenseRollupRow[]` (per-project × expenseId aggregate) cached at `tyro:dv:actualExpenseRollup`. Same refmap join, same excluded-codes / account-type-sign / distribution-wins model (see "Special-case rules in P&L math") — so per-project drill-downs and the Trade Cost aggregate always agree.

### Special-case rules in P&L math

- **Realised-expense belonging + sign model (calibrated to Power BI).** The per-project chain (`useProjectExpenseLines`) and the tenant rollup (`actualExpenseRollup`) share ONE model, verified to the dollar against a live Power BI screenshot on PRJ000002004 (1.160.392). Recalibrated totals after the price-diff fix: 2000=56.355, 2026=254.506, 2291=846.076, 2106=−247.643:
  - **Excluded codes** (`EXCLUDED_EXPENSE_IDS`, in BOTH files — keep in sync): taxes/treasury `730030 / 731016 / 790051 / 790052` **plus the futures-broker commission family `712207 / 712502`**. Power BI's realised report shows none of these. **Price-difference codes `710017 / 710041` are NOT excluded — PBI DOES count them** via the normal account-type sign. The earlier "PBI drops price differences" belief was wrong: it zeroed a +1.012.384 price-diff (710041) on 2004 while still counting its matching 720089 "MASRAF YANSITMA" reflection (−1.012.384) — an asymmetric wash that understated the project by that whole amount.
  - **Account-type sign** (`mserp_accounttype`): **Customer (200000001) = −1** (reflection / billed-back), **every other cost type = +1** — `COST_ACCOUNT_TYPES` = Vendor `200000003`, General accounting `200000000`, Bank `200000002`. Unknown/null → dropped. `mserp_isreturned` (Yes) flips the sign. (The old "only Vendor/Customer, drop the rest" rule wrongly dropped the General-accounting lines PBI includes.)
  - **Distribution-wins belonging:** a line belongs to the project when its `(expensenum, expenseid)` PAIR was distributed to it via the dist entity (`mserp_tryaifrtexpenselinedistlineentities`, keyed by the project's `mserp_inventdimid`s) — and that OVERRIDES the line's own `mserp_projectnum`. A cost can be booked under a "fixing" project (`FFIX…`, always out of `PROJECTS_FILTER` scope so it never double-claims) yet distributed to the voyage project. Only when NOT pair-distributed does projectnum apply (foreign → drop; empty/own → require the project in `mserp_defaultdimensiondisplayvalue`). The pair (not just the expensenum) also stops a sibling line on a shared voucher from leaking in.
...
- **i18n (TR/EN) is live — every user-facing string goes through `t()`.** Lightweight custom system: `useT()` from `src/lib/i18n/LanguageProvider.tsx` returns `t(key)`, resolving against `src/lib/i18n/translations.ts` (flat `tr` + `en` maps; missing key falls back TR → raw key, never blank). Default language Turkish; sidebar `LanguageToggle` (next to `ThemeSwitcher`, persisted under `tyro:lang`) flips to English. **CONVENTION (enforce on ALL new work): never hardcode display text — add the string to BOTH the `tr` and `en` maps with a namespaced key and render `t("key")`.** Code + comments stay English; data values (project/vessel/port/segment names, statuses, currencies, dates) are NOT translated. Module-level constants holding display strings (config arrays, selector-built text) carry KEY references resolved at the call site, or take `t`/`lang` as a param — see `plInsights.ts` (`generateSmartInsights(tree, t)`), Dashboard `KPI_META`, `entityConfig` `labelKey`/`hintKey`, RouteMap `stageLabel(stage, t)`. The TYRO AI assistant's reply language follows `lang` via `getSystemPrompt(lang)` in `src/lib/ai/systemPrompt.ts`. Deferred (still Turkish, low-priority): `fieldLabels.ts` raw inspector column names, `ProjectWebChat` Copilot-internal strings, `Intl.DateTimeFormat("tr-TR")` chart-axis month labels.
