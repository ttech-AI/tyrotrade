...
} from "@/freight/lib/dataverse/composeProjects";
  ...
  return React.useMemo<UseRealProjectsReturn>(() => {
    ...
    if (unresolved.length > 0) {
      ...
      if (fp !== lastWarningFingerprint) {
        // Module scope is deliberate and a ref would be wrong here: the point
        // is to warn ONCE per distinct set of unresolved ports across the
        // whole app, not once per component instance — several mounted
        // consumers would otherwise each re-log the same list.
...
function useCacheFingerprint(entitySet: string): string {
  ...
  React.useEffect(() => {
    ...
    const storageHandler = (e: StorageEvent) => {
      if (!e.key || e.key === `${KEY_PREFIX}${entitySet}`) {
