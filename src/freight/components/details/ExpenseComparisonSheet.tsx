...
import { ArrowLeftRight, ChevronRight } from "@/freight/icons";
 ...
 * tarayarak ayırabilsin): TAHMİNİ = marka bandı, GERÇEKLEŞEN = kâr (--pl-pos)
 * bandı. Satır tipografisi tamamen sans (mono YOK — kodlar yumuşak chip
...
/* Bölme kimlikleri — iki bölme birbirinden ayrılabilir kalmalı: TAHMİNİ
 * (plan) uygulamanın marka aksanını, GERÇEKLEŞEN (kapanmış para) K&Z kâr
 * jetonunu taşır. `solid` metin rengi olarak da kullanıldığı için marka
 * tarafında --brand-text seçildi (koyu temada açık tona döner). */
const EST = {
  solid: "var(--brand-text)",
  band: "color-mix(in oklab, var(--brand-via) 9%, transparent)",
  border: "color-mix(in oklab, var(--brand-via) 22%, transparent)",
};
const REAL = {
  solid: "var(--pl-pos)",
  band: "color-mix(in oklab, var(--pl-pos) 9%, transparent)",
  border: "color-mix(in oklab, var(--pl-pos) 22%, transparent)",
};
        ...
        <SheetHeader className="px-5 pt-5 pb-4 border-b border-border/40 text-left space-y-0">
          ...
          <div className="grid grid-cols-3 gap-2 pt-4">
            ...
            <SummaryStat
              ...
              color={overBudget ? "var(--pl-neg)" : "var(--pl-pos)"}
            />
          </div>
        </SheetHeader>
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Negative = reflection: it REDUCES the cost, i.e. money back —
            so it takes --pl-pos rather than --success. */}
        <span
          className={cn(
            "text-[13px] font-bold tabular-nums shrink-0",
            negative ? "text-pl-pos" : "text-foreground"
          )}
        >
          {negative ? "−" : ""}
      ...
      {open && (
        <div className="bg-foreground/[0.025] border-t border-border/40">
          {/* Realized-tone tree-rail + indentation tie the records to the
              parent and align them under its expand control; recessed bg +
              smaller muted type below make the child level read as clearly
              nested. */}
          <div
            className="ml-6 mr-2 my-1 border-l-2 pl-3 divide-y divide-border/30"
            style={{
              borderLeftColor:
                "color-mix(in oklab, var(--pl-pos) 30%, transparent)",
            }}
          >
    ...
    <div className="flex items-center justify-between gap-3 pr-1 py-1.5">
      ...
      <span
        className={cn(
          ...
          negative ? "text-pl-pos" : "text-foreground/65"
        )}
      >
        {negative ? "−" : ""}
...
function MatchBadge({ note }: { note: string }) {
  return (
    <span
      title={note}
      className="shrink-0 inline-flex items-center justify-center size-4 rounded-full bg-brand-via/12 text-brand"
    >
