...
import { AccentIconBadge, TONE_FORECAST } from "@/freight/components/details/AccentIconBadge";
import { useLocale } from "@/hooks/useLocale";
import { formatNumber, formatCurrency } from "@/freight/lib/format";
import type {
  RealizedPLMonthDetail,
  RealizedPLProjectRow,
} from "@/freight/lib/selectors/realizedPLTable";

/* Section tones. Projected rides the brand palette (its `grad` is the
 * brand-stop gradient, so the header strip carries the app's identity in
 * whichever palette is active); realized stays green so the two sections
 * remain mutually distinguishable in every palette. `solid` is the readable
 * text/chip accent. The realized ramp mixes toward white/black rather than
 * toward theme tokens so its deep end stays deep in dark mode too, keeping
 * the header's white text legible. */
const EST = {
  solid: "var(--series-projected)",
  grad: "linear-gradient(90deg, var(--brand-from) 0%, var(--brand-via) 60%, var(--brand-deep) 100%)",
  band: "color-mix(in oklab, var(--series-projected) 8%, transparent)",
  border: "color-mix(in oklab, var(--series-projected) 22%, transparent)",
};
const REAL = {
  solid: "var(--series-realized)",
  grad: "linear-gradient(90deg, color-mix(in oklab, var(--series-realized) 62%, white) 0%, var(--series-realized) 55%, color-mix(in oklab, var(--series-realized) 74%, black) 100%)",
  band: "color-mix(in oklab, var(--series-realized) 8%, transparent)",
  border: "color-mix(in oklab, var(--series-realized) 22%, transparent)",
};
/* These sign money, so they use the P&L tokens rather than success/destructive. */
const POS = "var(--pl-pos)";
const NEG = "var(--pl-neg)";
const NEUTRAL = "var(--pl-neutral)";
 ...
 * top, then two banded sections (Tahmini = brand accent, Gerçekleşen = green)
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span
            ...
            style={{
              color: tone,
              // `${tone}14` hex-alpha no longer works now that `tone` is a
              // CSS var expression.
