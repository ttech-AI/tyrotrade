 ...
 * floating-glass dialect with TyroAiDrawer (rounded-l-3xl, near-opaque
 * popover surface, theme accent strip on top). Body is a ScrollArea — each
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        ...
        className={cn(
          ...
          "shadow-[0_30px_80px_-16px_color-mix(in_oklab,var(--foreground)_45%,transparent)]"
        )}
 ...
 * total metric.
 *
 * Renders as a tinted band — soft foreground-tinted fill + soft inner border — so
    ...
    >
      ...
      {icon && (
        <span
          className={cn(
            ...
            "bg-popover/70 ring-1 ring-foreground/[0.06] text-foreground/80",
            toneColor ? "ml-1" : ""
          )}
          style={
            toneColor
              ? {
                  color: toneColor,
                  // `${toneColor}33` hex-alpha is gone now that tones are CSS vars.
                  boxShadow: `inset 0 0 0 1px color-mix(in oklab, ${toneColor} 20%, transparent)`,
                }
              : undefined
          }
        >
      ...
      <span
        className={cn(
          ...
          "bg-popover/85 text-foreground/75",
