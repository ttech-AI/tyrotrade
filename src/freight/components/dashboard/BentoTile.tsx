...
import type { IconBadgeTone } from "@/freight/components/details/AccentIconBadge";
...
export interface BentoTileProps extends BentoTileSpan {
   ...
   *  badge follows the live sidebar theme via `useBrandAccent`. */
          ...
          {(title || icon || headerAction) && (
            <header className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                {title && (
                  // Full-strength ink gives KPI tile titles a deliberate
                  // weight that lifts them off the glass surface without
                  // competing with the headline numbers below.
                  <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-foreground truncate">
                    {title}
                  </div>
                )}
                {subtitle && (
