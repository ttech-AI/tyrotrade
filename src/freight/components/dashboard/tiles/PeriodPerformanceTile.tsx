...
import { Loader2 } from "@/freight/icons";
...
import { useResolvedBrandAccent } from "@/freight/hooks/useBrandAccent";
...
import { aggregateEstimatedPL } from "@/freight/lib/selectors/aggregate";
import { getFinancialYear, type FinancialYear } from "@/freight/lib/dashboard/financialPeriod";
import { POWERBI_PL_BY_FY } from "@/freight/data/powerbiPL";
...
export function PeriodPerformanceTile({
  ...
  // Resolved (literal rgb) accent: recharts writes stroke/stopColor straight
  // onto SVG attributes, which reject var(). The hook re-resolves on palette
  // and theme changes.
  ...
  // These sign money, so the P&L tokens: profit → --pl-pos, loss → --pl-neg,
  // flat → --pl-neutral. Lands in a DOM style, so the raw var is fine.
  const plColor = (v: number) =>
    v > 0 ? "var(--pl-pos)" : v < 0 ? "var(--pl-neg)" : "var(--pl-neutral)";
              ...
              hasRealizedPL ? (
                ...
                <span className="inline-flex items-center gap-2 whitespace-nowrap self-center">
                  <Loader2
                    ...
                    style={{ color: "var(--series-realized)" }}
                  />
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                ...
                // The tooltip chrome is a plain DOM node, so CSS vars are
                // fine here — only the SVG props above need literals.
                contentStyle={{
