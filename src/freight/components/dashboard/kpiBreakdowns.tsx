...
import { selectProjectPL } from "@/freight/lib/selectors/profitLoss";
import {
  aggregateAvgTransitDays,
  aggregateByCorridor,
} from "@/freight/lib/selectors/aggregate";
import { toUsdAtDate } from "@/freight/lib/finance/fxRates";
import { formatCompactCurrency, formatTons } from "@/freight/lib/format";
  ...
  return (
    <div className="ml-3 mr-3 mb-2 flex items-center gap-2">
      <div className="flex-1 flex h-1 rounded-full overflow-hidden bg-foreground/[0.06]">
        {/* Categorical buckets: kept on three tokens that stay distinct AND
            legible in both themes — amber (navlun), brand accent (opex),
            neutral (diğer) — rather than collapsing onto one brand stop. */}
        {seg(freight, "var(--warning)")}
        {seg(opex, "var(--brand-text)")}
        {seg(other, "var(--muted-foreground)")}
      </div>
...
const STATUS_COLORS: Record<string, string> = {
  "To Be Nominated": "var(--status-tbn)",
  Nominated: "var(--status-nominated)",
  Commenced: "var(--status-commenced)",
...
/* Categorical, so the four keep four distinct tokens — same hue intent as
   before (green / brand / amber / neutral), now palette- and theme-aware. */
      ...
      <div className="flex flex-col gap-0.5">
        {rows.map(({ project: p, days }) => (
          <KpiProjectRow
            ...
            // Transit speed signs a state (slow / fast), not an amount, so
            // this is --destructive/--success rather than the P&L pair.
            metricColor={
              avg > 0 && days > avg * 1.3
                ? "var(--destructive)"
                : avg > 0 && days < avg * 0.7
...
/** Realized-P&L palette — kâr / zarar / nötr. Signs money, so the P&L tokens. */
function plColor(v: number): string {
  return v > 0 ? "var(--pl-pos)" : v < 0 ? "var(--pl-neg)" : "var(--pl-neutral)";
}
function plTint(v: number): string {
  return v > 0
    ? "color-mix(in oklab, var(--pl-pos) 12%, transparent)"
    : v < 0
      ? "color-mix(in oklab, var(--pl-neg) 12%, transparent)"
      : "color-mix(in oklab, var(--pl-neutral) 12%, transparent)";
}
  ...
  return (
    <div className="flex flex-col">
      <KpiGroupHeader
        ...
        valueChip={
          <span className="text-[10.5px] tabular-nums font-semibold text-warning">
